...
let RichTextBox1 = new RichTextBox(Top = 150,Width=200,Height=30)
...
let ClickButton _ = 
    ...
    if (D a b c)>0.0 then 
        ...
        RichTextBox1.Text<-"x1="+string(x1 a b c)+"\n"+" x2="+string(x2 a b c)
    elif (D a b c)=0.0 then
        let x (a:float) (b:float) (c:float) = (-b + Math.Sqrt(D a b c))/(2.0*a)
        RichTextBox1.Text<-"x1,2="+string(x a b c)
    else 
        RichTextBox1.Text<-"Нет решений"
